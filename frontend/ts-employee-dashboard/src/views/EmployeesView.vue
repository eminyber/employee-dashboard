...
const displayedPageSize = ref<number>(pageInfo.size);
...
const handleSortFieldChange = (newSortField: string): void => {
  filters.sortBy = newSortField;
};

const handleSearch = debounce((search: string): void => {
...
const handleJobTitleFilterChange = (appliedFilters: string[]): void => {
  filters.jobTitleIds = appliedFilters.join(",");
};

const handleSortOrderChange = (newSortOrder: string): void => {
